...
(function () {
  ...
  window.fetch = async function (...args) {
    ...
    if (url && (url.includes('.vtt') || url.includes('.srt') || url.includes('subtitle') || url.includes('caption'))) {
  ...
  XMLHttpRequest.prototype.open = function (method, url) {
    if (typeof url === 'string' && (url.includes('.vtt') || url.includes('.srt') || url.includes('subtitle') || url.includes('caption'))) {
      if (!isThumbnailUrl(url) && !window.__CAPTURED_SUB_URLS__.has(url)) {
        window.__CAPTURED_SUB_URLS__.add(url);
...
(function () {
  ...
  const IFRAME_ID = 'single-isolated-iframe-sandbox';
  const BRIDGE_ACTION_KEY = '__ROPHIM_CONTROL_SIGNAL__';

  const isInsideIframe = (window.self !== window.top) || (window.parent !== window);

  // =========================================================================
  // 🟢 LỚP 2: CHIẾN LƯỢC XỬ LÝ TRONG IFRAME (CHILD)
  // =========================================================================
  if (isInsideIframe) {
    if (window.__CHILD_CONTROLLER_INITIALIZED__) return;
    window.__CHILD_CONTROLLER_INITIALIZED__ = true;

    bridgeLog('[Lớp 2 - Iframe] ⚡ Khởi chạy Controller điều khiển Video...');

    function notifyParent(message) {
      try {
        window.parent.postMessage({
          type: BRIDGE_ACTION_KEY,
          action: 'SHOW_TOAST',
          message: message
        }, '*');
      } catch(e) {}
    ...
    // A. LẮNG NGHE PHÍM PAGE-DOWN CỦA REMOTE ĐỂ QUA TẬP
    function setupRemoteKeyHandler() {
      window.addEventListener('keydown', function (e) {
        if (e.key === 'PageDown' || e.keyCode === 34) {
          bridgeLog('[Lớp 2 - Remote] 🔘 Nhận tín hiệu phím PageDown!');
          const nextBtn = document.querySelector('.item-btn.item-next');
          if (nextBtn) {
            bridgeLog('[Lớp 2 - Remote] ⏩ Đã tìm thấy nút Next, kích hoạt Click!');
            nextBtn.click();
            notifyParent('⏩ Đã chuyển sang tập tiếp theo');
          } else {
            // Nếu không tìm thấy trong Lớp 2, gửi signal ra Lớp 1 tìm thử
            try {
              window.parent.postMessage({
                type: BRIDGE_ACTION_KEY,
                action: 'TRIGGER_NEXT_EPISODE'
              }, '*');
            } catch(err) {}
          }
        }
    ...
    // B. QUAN SÁT VÀ AUTO-CLICK #btn-close-resume TRONG 1 PHÚT
    function startElementWatcher() {
      bridgeLog('[Lớp 2 - AutoClick] 👁️ Bắt đầu âm thầm quan sát các Element trong 60s...');
      ...
      const watcherInterval = setInterval(() => {
        ...
        if (elapsedTime > 60) {
          ...
          bridgeLog('[Lớp 2 - AutoClick] ⏱️ Hết 60s theo dõi Element.');
          return;
        }

        // Kiểm tra nút #btn-close-resume
        const closeResumeBtn = document.querySelector('#btn-close-resume');
        if (closeResumeBtn && closeResumeBtn.offsetParent !== null) { // Kiểm tra element đang hiển thị
          bridgeLog('[Lớp 2 - AutoClick] 🎯 Phát hiện nút #btn-close-resume! Đang tự động Click...');
    ...
    function setupVideoController() {
      ...
      if (!video) {
        setTimeout(setupVideoController, 500);
        return;
      }

      bridgeLog('[Lớp 2 - Iframe] 🎥 Đã phát hiện thẻ Video DOM!');

      // C. ÉP BẬT SUBTITLE TIẾNG VIỆT
      function enableVietnameseSubtitles() {
        try {
          if (video.textTracks && video.textTracks.length > 0) {
            for (let i = 0; i < video.textTracks.length; i++) {
              ...
              if (lang.includes('vi') || lang.includes('viet') || lang.includes('tiếng việt')) {
                track.mode = 'showing';
                bridgeLog('[Lớp 2 - Iframe] 💬 Đã bật Phụ đề Tiếng Việt!');
                notifyParent('💬 Đã bật phụ đề Tiếng Việt');
                return;
              }
            }
          }
          ...
          if (window.jwplayer && typeof window.jwplayer === 'function') {
            ...
            if (player && player.getTracks) {
              const tracks = player.getTracks();
              const viIndex = tracks.findIndex(t => (t.label || '').toLowerCase().includes('vi'));
              if (viIndex !== -1) {
                player.setCurrentCaptions(viIndex);
                bridgeLog('[Lớp 2 - Iframe] 💬 Đã bật Phụ đề Tiếng Việt (JWPlayer)!');
                notifyParent('💬 Đã bật phụ đề Tiếng Việt');
                return;
              }
            }
          ...
          bridgeLog('[Lớp 2 - Iframe] ⚠️ Chưa thể bật phụ đề: ' + e.message);
        }
      }

      // D. CHỌN ĐỘ PHÂN GIẢI CAO NHẤT (SMART SELECTOR)
      function setHighestQuality() {
        try {
          if (window.jwplayer && typeof window.jwplayer === 'function') {
            const player = window.jwplayer();
            if (player && player.getQualityLevels) {
              const levels = player.getQualityLevels();
              ...
              for (const group of priorityKeywords) {
                selectedIndex = levels.findIndex(level => {
                  const label = (level.label || '').toLowerCase();
                  return group.some(kw => label.includes(kw));
                });
              ...
              if (selectedIndex === -1) {
                ...
                levels.forEach((level, index) => {
                  let res = level.height || 0;
                  if (!res && level.label) {
                    const match = level.label.match(/(\\d+)/);
                    if (match) res = parseInt(match[1]);
                  }
                  if (res > maxRes) {
                    maxRes = res;
                    selectedIndex = index;
              ...
              player.setCurrentQuality(selectedIndex);
              const qualityLabel = levels[selectedIndex].label || 'Cao nhất';
              bridgeLog('[Lớp 2 - Iframe] 📺 Đã chỉnh độ phân giải: ' + qualityLabel);
              notifyParent('📺 Đã chỉnh độ phân giải: ' + qualityLabel);
            }
          }
        } catch (e) {
          bridgeLog('[Lớp 2 - Iframe] ⚠️ Không thể chỉnh chất lượng: ' + e.message);
        }
      }

      // E. CHỐNG PAUSE TRONG 1 PHÚT ĐẦU
      function startAutoplayGuard() {
        bridgeLog('[Lớp 2 - Iframe] 🛡️ Kích hoạt chế độ ép Play 60s đầu...');
        const startTime = Date.now();
        ...
        const guardInterval = setInterval(() => {
          ...
          if (elapsedTime > 60) {
            ...
            bridgeLog('[Lớp 2 - Iframe] ⏱️ Đã hết 60s kiểm tra Play.');
            return;
          }

          if (video.paused && video.readyState >= 2) {
            bridgeLog('[Lớp 2 - Iframe] 🔄 Phát hiện Video bị Pause, đang ép Play lại...');
            video.play().catch(() => {
              video.muted = true;
              video.play();
            });
    ...
    // Kích hoạt các trình lắng nghe ở Lớp 2
    ...
    if (document.readyState === 'complete') {
      ...
      window.addEventListener('load', setupVideoController, { once: true });
    }
  ...
  // =========================================================================
  // 🔵 LỚP 1: TRANG CHÍNH QUAN SÁT VÀ ĐIỀU HÀNH (PARENT)
  // =========================================================================
  try {
    if (window.__SANDBOX_CREATED__ || document.getElementById(IFRAME_ID)) {
      bridgeLog('[Lớp 1 - Parent] 🛑 Đã chặn injection trùng lặp!');
      return;
    }
  } catch(e) {}

  window.__SANDBOX_CREATED__ = true;
  bridgeLog('[Lớp 1 - Parent] 🚀 Đang dựng Sandbox Lớp 1...');

  function showMiniToast(text) {
    try {
      let toastContainer = document.getElementById('sandbox-toast-container');
      
      if (!toastContainer) {
        toastContainer = document.createElement('div');
        toastContainer.id = 'sandbox-toast-container';
  ...
  // Lắng nghe Remote PageDown từ Lớp 1
  window.addEventListener('keydown', function (e) {
    if (e.key === 'PageDown' || e.keyCode === 34) {
      bridgeLog('[Lớp 1 - Remote] 🔘 Nhận tín hiệu PageDown trên Lớp 1!');
  ...
  function initParentSandbox() {
    if (document.getElementById(IFRAME_ID)) return;

    try {
      if (document.head) document.head.innerHTML = '<meta charset="utf-8">';
      if (document.body) {
      ...
      bridgeLog('[Lớp 1 - Parent] ✅ Đã khởi tạo Lớp 2 thành công! URL: ' + TARGET_URL);

      // LẮNG NGHE POSTMESSAGE TỪ LỚP 2
      window.addEventListener('message', function (event) {
        if (event.data && event.data.type === BRIDGE_ACTION_KEY) {
          if (event.data.action === 'SHOW_TOAST' && event.data.message) {
            showMiniToast(event.data.message);
          } else if (event.data.action === 'TRIGGER_NEXT_EPISODE') {
            const nextBtn = document.querySelector('.item-btn.item-next');
            if (nextBtn) {
              nextBtn.click();
              showMiniToast('⏩ Đã chuyển tập tiếp theo');
            }
          }
        }
      });

      // GIÁM SÁT THAY ĐỔI LINK CỦA LỚP 2 (MUTATION OBSERVER & IN LOG)
      ...
      const observer = new MutationObserver((mutations) => {
        for (const mutation of mutations) {
          if (mutation.type === 'attributes' && mutation.attributeName === 'src') {
            ...
            if (newSrc !== lastSrc) {
              bridgeLog('[Lớp 1 - Parent] 🔄 [URL CHANGED] Iframe đã thay đổi link thành: ' + newSrc);
              lastSrc = newSrc;
              showMiniToast('🔄 Đã chuyển tập phim mới');

              try {
                iframe.contentWindow.postMessage({ type: BRIDGE_ACTION_KEY, action: 'RESTART_SCRIPT' }, '*');
              } catch(e) {}
            }
          }
        }
      ...
      bridgeLog('[Lớp 1 - Parent] ❌ Lỗi khởi tạo: ' + err.message);
    }
  }
  ...
  if (document.body) {
    ...
    document.addEventListener('DOMContentLoaded', initParentSandbox, { once: true });
  }
