...
function parseMovieDetail(html, url) {
    ...
    try {        
        ...
        // --- BẮT ĐẦU SẮP XẾP SERVER ƯU TIÊN (English > HLS > 1080p) ---
        function getServerScore(serverName) {
            var nameLower = String(serverName).toLowerCase();
            var score = 0;

            if (nameLower.indexOf('english') > -1 || nameLower.indexOf('eng') > -1 || nameLower.indexOf('en') > -1) {
                score += 100;
            }
            if (nameLower.indexOf('hls') > -1 || nameLower.indexOf('m3u8') > -1) {
                score += 10;
            }
            if (nameLower.indexOf('1080') > -1) {
                score += 1;
            }
        ...
        if (Array.isArray(availableServers) && availableServers.length > 0) {
            availableServers.sort(function(a, b) {
                return getServerScore(b) - getServerScore(a);
            });
        }
        // --- KẾT THÚC SẮP XẾP SERVER ---
            ...
            if (isTV && $data.seasons && Array.isArray($data.seasons)) {
                ...
                if (baseEpisodes.length > 0) {
                    availableServers.forEach(function(serverName, index) {
                        var serverIndex = index + 1; // Sử dụng index bắt đầu từ 1
                        
                        var serverEpisodes = baseEpisodes.map(function(ep) {
                            var epId = BASEAPI + "/api/"+BASESV+"/?mediaType=tv&id=" + tmdbId + "&season=" + ep.season + "&episode=" + ep.episode + "&source=all&debug=9780752" + "&server=" + serverIndex;
                ...
                availableServers.forEach(function(serverName, index) {
                    var serverIndex = index + 1; // Sử dụng index bắt đầu từ 1
                    var idMovie = BASEAPI + "/api/"+BASESV+"/?mediaType=movie&id=" + tmdbId + "&source=all&debug=9780752" + "&server=" + serverIndex;
...
function parseDetailResponse(html, url) {
  try {
    ...
    // Trích xuất tham số server từ URL (ví dụ: &server=1 hoặc &server=10)
    var serverParamMatch = url.match(/[?&]server=(\d+)/i);
    var targetServerIndex = serverParamMatch ? parseInt(serverParamMatch[1], 10) : null;

    // Xử lý vị trí Server dựa trên index
    if (targetServerIndex !== null && validSources.length > 0) {
      var selectedIdx = targetServerIndex - 1; // Đổi về 0-based index

      if (selectedIdx < 0 || selectedIdx >= validSources.length) {
        console.log("⚠️ server=" + targetServerIndex + " vượt quá danh sách (" + validSources.length + " sources). Fallback về server 1.");
        selectedIdx = 0;
      }

      var chosenSource = validSources[selectedIdx];
      var remainingSources = validSources.filter(function(_, idx) {
        return idx !== selectedIdx;
      });

      validSources = [chosenSource].concat(remainingSources);
    }
    ...
    if (validSources.length > 0) {
      // Định danh Key theo ID + Season + Episode
      var idMatch = url.match(/[?&]id=([^&]+)/i);
      var seasonMatch = url.match(/[?&]season=(\d+)/i);
      var epMatch = url.match(/[?&]episode=(\d+)/i);

      mediaId = idMatch ? idMatch[1] : "default";
      season = seasonMatch ? seasonMatch[1] : "0";
      episode = epMatch ? epMatch[1] : "0";
      ...
      if (selectedSource.headers) {
        try {
          ...
          console.log("Lỗi parse headers từ source:", e);
        }
      }
    ...
    if (viCount == 0) {
      ...
      if (season && season !== "0") {
        type = "tv";
        var linksub = "https://floral-brook-42fb.alokillgtv.workers.dev/?url=https%3A%2F%2Fsubs.1shows.app%2Fdata%2Fsubs%2F" + type + "%2F" + mediaId + "%2F" + season + "%2F" + episode;
      } else {
        var linksub = "https://floral-brook-42fb.alokillgtv.workers.dev/?url=https%3A%2F%2Fsubs.1shows.app%2Fdata%2Fsubs%2F" + type + "%2F" + mediaId;
      }       
      console.log("sub dự phòng: \n" + linksub)
      subtitleList.push({
        lang: "Engsub "+ (enCount + 1)+" (Dịch Dự Phòng)",
        url: linksub,
        mimeType: "text/vtt"
      });
    }

    // 4. Bổ sung/Ghi đè Referer từ dữ liệu nguồn vào Final Headers
    var defaultReferer = "https://vidgod.site";
    var defaultOrigin = "https://vidgod.site";

    // Nếu trong sourceHeaders có Referer thì lấy nó, ngược lại dùng defaultReferer
    var customReferer = sourceHeaders["Referer"] || sourceHeaders["referer"] || defaultReferer;
    
    // Tự động tính toán Origin dựa trên Referer truyền vào (nếu có)
    var customOrigin = defaultOrigin;
    try {
...
function parseEmbedResponse(html, url) {
    ...
    try {
      ...
      return JSON.stringify({
        ...
        headers: {
          "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
          "Referer": "https://vidgod.site",
          "Origin": "https://vidgod.site",
          "Block-Ads": false,
          "Block-Css": "",
          "Custom-Js": customJS
        },
